...
if (isset($_POST['config_content']) && isset($_POST['save_config'])) {
    if (isset($selected_config) && !empty($selected_config)) {
...
if (isset($_POST['action'])) {
    ...
    if ($action_type == 'load_proxy') {
        $proxyPath = $_POST['proxycfg'] ?? '';
        if (file_exists($proxyPath) && is_path_allowed($proxyPath, $allowed_dirs)) {
            echo file_get_contents($proxyPath);
        }
        exit;
    }

    if ($action_type == 'save_proxy') {
        $proxyPath = $_POST['proxycfg'] ?? '';
        $content   = $_POST['content'] ?? '';
        if (file_exists($proxyPath) && is_path_allowed($proxyPath, $allowed_dirs)) {
            file_put_contents($proxyPath, $content);
            header('Content-Type: application/json');
            echo json_encode(['status' => 'success', 'icon' => 'success', 'title' => 'Saved!', 'message' => 'Proxy successfully saved']);
        } else {
            header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'icon' => 'error', 'title' => 'Error!', 'message' => 'File not found or access denied']);
        }
        exit;
    }

    if ($action_type == 'load_rules') {
        $rulesPath = $_POST['rulescfg'] ?? '';
        if (file_exists($rulesPath) && is_path_allowed($rulesPath, $allowed_dirs)) {
            echo file_get_contents($rulesPath);
        }
        exit;
    }

    if ($action_type == 'save_rules') {
        $rulesPath = $_POST['rulescfg'] ?? '';
        $content   = $_POST['content'] ?? '';
        if (file_exists($rulesPath) && is_path_allowed($rulesPath, $allowed_dirs)) {
            file_put_contents($rulesPath, $content);
            header('Content-Type: application/json');
            echo json_encode(['status' => 'success', 'icon' => 'success', 'title' => 'Saved!', 'message' => 'Rules successfully saved']);
        } else {
            header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'icon' => 'error', 'title' => 'Error!', 'message' => 'File not found or access denied']);
        }
...
if (isset($_POST['clashconfig'])) {
    ...
    // Validasi path: harus dalam allowed_dirs
    if (is_path_allowed($dt, $allowed_dirs) || (file_exists($dt) && strpos(realpath($dt), '/etc/neko/') === 0)) {
        shell_exec("uci set neko.cfg.selected_config=" . escapeshellarg($dt) . " && uci commit neko");
        $selected_config = $dt;
    }
}

if (isset($_POST['neko']) && $_POST['neko'] == 'apply') {
...
if (isset($_POST['clashconfig']) && !isset($_POST['neko'])) {
    header('Location: configs.php');
    exit;
}
...
if (isset($_POST["file_action"])) {
    $action_str = $_POST["file_action"];
    $parts      = explode("@", $action_str, 2);
    $command    = $parts[0] ?? '';
                ...
                <div class="mb-4">
                    <div class="col input-group">
                        <select class="form-select form-select-lg" name="clashconfig" aria-label="themex">
                            ...
                            <option value="<?php echo $file ?>" <?php if($file === $selected_config) echo 'selected'; ?>><?php echo $file ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div id="info" class="tab-pane fade show active">
                    ...
                    <div class="container mb-5">
                        ...
                        <div class="d-md-none">
                            <div class="row g-2">
                                ...
                                <div class="col-6">
                                    ...
                                    <input class="form-control form-control-sm text-center" name="ech" type="text" placeholder="<?php echo $neko_cfg['enhanced'] ?>" disabled>
                                </div>
                                <div class="col-6">
                                    <label class="form-label text-center w-100">
                        ...
                        <h2 class="text-center p-2 mt-4">
                            <?php echo ($core_mode == 'singbox') ? 'Singbox Config' : 'Mihomo Config'; ?>
                        </h2>
                            ...
                            <div class="row justify-content-md-center">
                                <div class="col input-group mb-3 justify-content-md-center">
                                    <select class="form-select" id="proxySelect" name="proxycfg" aria-label="themex">
                                        ...
                                        $proxyFiles = array_merge(glob("$proxyDirPath/*.yaml") ?: [], glob("$proxyDirPath/*.txt") ?: [], glob("$proxyDirPath/*.list") ?: []);
                            ...
                            <div class="row justify-content-md-center">
                                <div class="col input-group mb-3 justify-content-md-center">
                                    <select class="form-select" id="rulesSelect" name="rulescfg" aria-label="themex">
                                        ...
                                        <?php
                                        $ruleFiles = array_merge(glob("$ruleDirPath/*.yaml") ?: [], glob("$ruleDirPath/*.txt") ?: [], glob("$ruleDirPath/*.list") ?: []);
                            ...
                            <div class="alert alert-info alert-dismissible fade show p-2" role="alert">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="flex-grow-1 ms-2">
                                        <small class="d-block mb-1"><i data-feather="shield" class="feather-xs me-1"></i><strong>TROJAN:</strong> GFW, WS TLS/NTLS, GRPC</small>
                                        <small class="d-block mb-1"><i data-feather="box" class="feather-xs me-1"></i><strong>VMESS:</strong> WS TLS/NTLS, HTTP, H2, GRPC</small>
                                        <small class="d-block"><i data-feather="lock" class="feather-xs me-1"></i><strong>VLESS:</strong> WS TLS/NTLS, XTLS, GRPC</small>
                                        <small class="d-block"><i data-feather="key" class="feather-xs me-1"></i><strong>SS:</strong> DIRECT, OBFS, V2RAY/XRAY-PLUGIN</small>
                                        <small class="d-block"><i data-feather="link" class="feather-xs me-1"></i><strong>SUBSCRIBE LINK:</strong> Only works with active internet connection</small>
                                    </div>
                                </div>
                ...
                <div id="manager" class="tab-pane fade">
                    ...
                    <div class="container mb-5">
                        ...
                        <div class="container container-bg border border-3 rounded-4 col-12 mb-4">
                            ...
                            <form id="uploadForm" action="configs.php" method="POST" enctype="multipart/form-data">
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td class="col-2">
                                                <select class="form-select" name="path_selector">
                                                    ...
                                                    <option value="BACKUP CONFIG">BACKUP CONFIG</option>
                                                    <option value="RESTORE CONFIG">RESTORE CONFIG</option>
                                                    <!-- FIX: $arrPath sudah didefinisikan di atas -->
                                                    <?php foreach ($arrPath as $dir) echo "<option value=\"".$dir.'">'.$dir."</option>"; ?>
                                                </select>
                                            </td>
                                            <td class="col-2">
...
<script>
// Mihomo-only listeners — guard dengan null check agar tidak crash saat singbox
if (document.getElementById('selectProxy')) {
...
if (document.getElementById('saveProxy')) {
    document.getElementById('saveProxy').addEventListener('click', function(e) {
        ...
        .then(data => {
            Swal.fire({ icon: data.icon, title: data.title, text: data.message, toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
        })
        .catch(() => {
            Swal.fire({ icon: 'error', title: 'Error!', text: 'Failed to save proxy', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
        });
    });
}

if (document.getElementById('selectRules')) {
...
if (document.getElementById('saveRules')) {
    document.getElementById('saveRules').addEventListener('click', function(e) {
        ...
        .then(data => {
            Swal.fire({ icon: data.icon, title: data.title, text: data.message, toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
            if (data.status === 'success') feather.replace();
        })
        .catch(() => {
            Swal.fire({ icon: 'error', title: 'Error!', text: 'Failed to save rules', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
        });
    });
}
...
document.querySelector('a[href="#manager"]') && document.querySelector('a[href="#manager"]').addEventListener('shown.bs.tab', function(e) {
    loadManagerTables();
    loadManagerModals();
});
...
document.getElementById('uploadForm').addEventListener('submit', function(e) {
    ...
    if (pathSelector === 'BACKUP CONFIG') {
        fetch('configs.php', { method: 'POST', body: formData })
        .then(response => {
        ...
        .then(data => {
            if (data && data.status) {
                Swal.fire({ icon: data.icon, title: data.title, text: data.message, showConfirmButton: false, timer: 1500, position: 'top-end', toast: true });
            }
        })
...
function refreshTables() {
    ...
    .then(data => {
        ...
        return fetch('manager.php', {
            method: 'POST',
            body: new URLSearchParams({ 'action': 'get_modals' })
        });
    })
    .then(response => response.json())
    .then(data => {
        let modalContainer = document.getElementById('managerModals');
        if (modalContainer) {
...
function handleFileAction(action) {
    ...
    if (action.startsWith('save@')) {
        ...
        // Konsisten dengan manager.php: str_replace('.', '_', basename($file))
        const fileBase = filePath.split('/').pop().replace(/\./g, '_');
        const textarea = document.getElementById('content_' + fileDir + '_' + fileBase);

        if (!textarea) {
            console.error('Textarea not found: content_' + fileDir + '_' + fileBase);
            return;
        }
    ...
    if (action.startsWith('del@')) {
        Swal.fire({
            ...
            if (result.isConfirmed) sendRequest(formData);
        });
        return;
    }
...
function sendRequest(formData) {
    ...
    .catch(error => {
        Swal.fire({ icon: 'error', title: 'Error!', text: 'An error occurred: ' + error.message, position: 'top-end', toast: true, timer: 3000 });
    });
}
...
function saveFile(filePath, contentId) {
    ...
    .catch(error => {
        Swal.fire({ icon: 'error', title: 'Error!', text: 'System error occurred', toast: true, position: 'top-end', timer: 3000 });
    });
}
...
function deleteFile(filePath) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .then(response => response.json())
            .then(data => {
                Swal.fire({ icon: 'success', title: 'Deleted!', text: 'File successfully deleted', toast: true, position: 'top-end', timer: 3000, showConfirmButton: false });
                bootstrap.Modal.getInstance(document.querySelector('.modal.show')).hide();
                loadManagerTables();
            });
        }
