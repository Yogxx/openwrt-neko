...
function restoreConfig() {
    ...
    $restore  = "/tmp/" . $name;
    $test_dir = "/tmp/neko_restore_test_" . uniqid();

    move_uploaded_file($tmp, $restore);

    // Validasi isi archive sebelum hapus direktori existing
    $test_result = shell_exec("mkdir -p " . escapeshellarg($test_dir) . " && tar -xzf " . escapeshellarg($restore) . " -C " . escapeshellarg($test_dir) . " 2>&1");
    $has_config  = is_dir("$test_dir/config") || glob("$test_dir/config/*");

    if (!$has_config) {
        shell_exec("rm -rf " . escapeshellarg($test_dir));
        unlink($restore);
        echo json_encode(['status' => 'error', 'message' => 'Invalid backup file: config directory not found']);
        exit;
    }

    // Validasi OK — sekarang baru replace
    shell_exec("rm -rf /etc/neko/config /etc/neko/proxy_provider /etc/neko/rule_provider 2>/dev/null");
    shell_exec("cp -r " . escapeshellarg($test_dir) . "/. /etc/neko/ 2>/dev/null");
