...
if (!empty($selected_config) && file_exists($selected_config)) {
    ...
    if ($core_mode === 'singbox') {
        ...
        $neko_cfg['redir']          = preg_match('/^\s*redir-port:\s*(\S+)/m',         $yaml, $m) ? $m[1] : '';
        $neko_cfg['port']           = preg_match('/^port:\s*(\S+)/m',                   $yaml, $m) ? $m[1] : '';
        $neko_cfg['socks']          = preg_match('/^\s*socks-port:\s*(\S+)/m',         $yaml, $m) ? $m[1] : '';
        $neko_cfg['mixed']          = preg_match('/^\s*mixed-port:\s*(\S+)/m',         $yaml, $m) ? $m[1] : '';
        $neko_cfg['tproxy']         = preg_match('/^\s*tproxy-port:\s*(\S+)/m',        $yaml, $m) ? $m[1] : '';
        $neko_cfg['mode']           = preg_match('/^mode:\s*(\S+)/m',                   $yaml, $m) ? strtoupper($m[1]) : '';
        $neko_cfg['enhanced']       = preg_match('/^\s*enhanced-mode:\s*(\S+)/m',      $yaml, $m) ? strtoupper($m[1]) : '';
        $neko_cfg['secret']         = preg_match('/^secret:\s*(\S+)/m',                 $yaml, $m) ? $m[1] : '';
        $neko_cfg['ext_controller'] = preg_match('/^external-controller:\s*"?(\S+?)"?$/m', $yaml, $m) ? $m[1] : '';
    }
}
